...
const BUTTON_CLASS = 'min-h-12 rounded-full border border-[#1d1d1f] px-[22px] font-extrabold disabled:cursor-not-allowed disabled:opacity-45';
const SECONDARY_BUTTON_CLASS = `${BUTTON_CLASS} bg-transparent text-inherit`;
const PRIMARY_BUTTON_CLASS = `${BUTTON_CLASS} bg-[#1d1d1f] text-white`;
...
export function BackupPanel() {
  ...
  return (
    <>
      ...
      <p className={HINT_CLASS} id="backup-status" role="status" aria-live="polite">
        {backup.backupStatus}
      </p>

      <div className="mt-7 border-t border-[rgba(29,29,31,.1)] pt-6" aria-labelledby="data-reset-title">
        <div className="mb-3 flex items-baseline justify-between gap-3">
          <h3 className="m-0 text-[0.95rem]" id="data-reset-title">この端末のONEデータ</h3>
          <span className="text-[0.72rem] font-bold text-[#77736a]">プライバシー</span>
        </div>
        ...
        <p className={HINT_CLASS} id="data-reset-status" role="status" aria-live="polite">
          {reset.resetStatus}
        </p>
      </div>
    </>
