...
def main():
    ...
    if ".task-card" in styles or "task-storage-status" in styles:
        raise SystemExit("ERROR: 廃止したタスク入力向けスタイルを残さないでください")

    focus_timer_rules = (
        "body.focus-mode .timer-card {",
        "body.focus-mode .timer-card .section-heading,",
        "body.focus-mode .timer-card .presets,",
        "body.focus-mode .timer-card .hint {",
        "body.focus-mode .timer {",
    )
    for rule in focus_timer_rules:
